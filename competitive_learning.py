...
def mainloop(leaky=False, dinit=False, neighbours=False):
    ...
    theta = 5E-5 # Learning rate of other units. An order of magnitude smaller.
    ...
    normW = normW.reshape(digits, -1) # -1 means that the dimension is inferred.
    ...
    extent = axes_stats[3].get_window_extent().transformed(fig_stats.dpi_scale_trans.inverted())
    fig_stats.savefig('average_weight_changes.pdf', bbox_inches=extent.expanded(1.1, 1.2))
